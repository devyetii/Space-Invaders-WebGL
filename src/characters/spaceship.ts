...
import gameScene from '../project_scenes/010-gameScene'
...
export  class  player extends spaceShip {
    ...
    score:number;
    public constructor(pos: vec3, speed:number, width:number, height:number, length:number, shootPower:number, direction:vec3, health:number, mygameScene:gameScene){
    ...
    public draw(VP: mat4, program:ShaderProgram): void {
      ...
      // mat4.rotateX(spaceshipMat, spaceshipMat, Math.PI/2);
       mat4.translate(spaceshipMat, spaceshipMat, [0, 10   , 0]);
       mat4.scale(spaceshipMat, spaceshipMat,vec3.fromValues(5,5,5));
